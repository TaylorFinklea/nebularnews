...
export async function generateScheduledBriefs(env: Env): Promise<void> {
  ...
  for (const { user_id } of enabledUsers) {
    try {
      ...
      // Check if current UTC hour matches either configured time.
      // (Timezone handling is simplified — uses UTC. User's timezone offset should be applied in a future iteration.)
      ...
      if (currentHour === morningHour && currentMinute < 30) editionType = 'morning';
      else if (currentHour === eveningHour && currentMinute < 30) editionType = 'evening';
      ...
      const existing = await dbGet<{ id: string }>(
        ...
        [user_id, editionType, dayStart.getTime()],
      );
