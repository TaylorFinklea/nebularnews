...
export const POST = async ({ request, platform, locals }) => {
  try {
    ...
    logInfo('onboarding.subscribe.start', { user_id: user.id, feed_count: feedUrls.length });
    ...
    const pull = await startManualPull(db, { cycles: 1, trigger: 'onboarding', requestId: null });

    logInfo('onboarding.subscribe.success', { user_id: user.id, subscribed, run_id: pull.runId });
